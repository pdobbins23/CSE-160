...
function terrainHeight(x, z) {
	...
	const clearingRadius = 22;
	const baseH = 4;

	if (dist < clearingRadius) return baseH;

	const f1 = Math.sin(x * 0.06) * Math.cos(z * 0.07) * 1.2;
	const f2 = Math.sin(x * 0.03 + 1.3) * 1.0;
	const f3 = Math.cos(z * 0.027 - 0.7) * 1.0;

	const ramp = Math.min(1, (dist - clearingRadius) / 8);
	const hills = (f1 + f2 + f3) * 1.5 * ramp;
	const h = Math.round(baseH + ramp * 3 + hills);
...
class World {
	...
	render(time) {
		...
		drawEmissiveCube(sky, COLOR_SKY);
